...
<template>
  <div id="sidebar-wrap" :class="{ collapsed: toggleSideBar }">
    <!-- 顶部图标 -->
    <h3 class="logo">
      <span>{{toggleSideBar || langType === 'en' ? 'CDFZ': '江财附中'}}</span>
    </h3>

    <!-- 循环生成侧栏菜单 -->
    <!-- <el-menu background-color="#3f4d67" text-color="#fff" :default-active="$route.path" :unique-opened="true" :router="true" mode="vertical" :collapse="toggleSideBar"> -->
    <el-menu :default-active="$route.path" :unique-opened="true" :router="true" mode="vertical" :collapse="toggleSideBar">
